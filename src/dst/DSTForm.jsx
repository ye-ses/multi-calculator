    ...
    <STDcontainer>
      ...
      {speedCheck && (
        <>
          <Accordion
            Aname="Distance"
            lbl1="Enter the distance given"
            lbl2="Select the distance unit"
            opts={distanceOptions}
            vFunk={changeDistance}
            uFunk={changeDistanceUnit}
            value={distance}
            u={dUnit}
          />
          <Accordion
            Aname="Time"
            lbl1="Enter the time value given"
            lbl2="Select the time unit"
            opts={timeOptions}
            vFunk={changeTime}
            uFunk={changeTimeUnit}
            value={time}
            u={tUnit}
          />
          <Answers>
            <h3>
              According to the values you entered,and/ selected, below are the
              results....
            </h3>

            <p>
              For a given distance of{" "}
              <span>
                {evaluator.distance} {evaluator.dUnit}
              </span>{" "}
              ,(distance is always converted to meters)
            </p>
            <p>
              and the time of{" "}
              <span>
                {evaluator.time} {evaluator.tUnit}
              </span>{" "}
              ,(time is always converted to seconds)
            </p>
            <p>
              The speed it would take to cover the distance at the specified
              time will be{" "}
              <span>
                {evaluator.speed}
                {"  "}
      ...
      {distanceCheck && (
        <>
          ...
          <Accordion
            Aname="Speed"
            lbl1="Enter the speed given"
            lbl2="Select the speed unit"
            opts={speedOptions}
            vFunk={changeSpeed}
            uFunk={changeSpeedUnit}
            value={speed}
            u={sUnit}
          />
          <Answers>
            ...
            <p>
              and the speed of{" "}
              <span>
                {evaluator.speed} {evaluator.sUnit}
              </span>{" "}
      ...
      {timeCheck && (
        <>
          ...
          <Answers>
            ...
            <p>
              The time it would take to cover the distance will be{" "}
              <span>
                {evaluator.time} {"  "}
                {evaluator.tUnit}
              </span>
